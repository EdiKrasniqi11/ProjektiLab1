...
export class Vleresimi extends Component{
    ...
    refreshList(){
        ...
        .then(data=>{this.setState({lendet: data}); this.setState({lendetFilter: data})})
    ...
    changeStudenti = (e) =>{
        ...
        var selProvimet = this.state.provimet.filter(provimi => provimi.Studenti == e.target.value);
        var count = 0
        var lendet = []
        selProvimet.map(provimet => lendet[count++] = provimet.Lenda)
        var selLendet = this.state.lendet.filter(lenda => lendet.includes(lenda.LendaID))
        this.setState({lendetFilter: selLendet})
